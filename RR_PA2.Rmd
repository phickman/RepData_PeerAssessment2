...
And injuries by event type

```{r}
injuryByType <- aggregate(INJURIES ~ EVTYPE, data = data, sum, na.rm = TRUE)
injuryByType <- injuryByType[order(injuryByType$INJURIES, decreasing = TRUE), ]

# show just the top 10
injuryByType <- injuryByType[1:10,]

barplot(injuryByType$INJURIES, names.arg = injuryByType$EVTYPE)
